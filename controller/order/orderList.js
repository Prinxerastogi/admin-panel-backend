...
let getOrderCount = (req, res, next) => {
  ...
  if (req.query.startDate && req.query.endDate) {
    ...
    endDate = new Date();
    //    endDate.setHours(0, 0, 0, 0); // Reset to the start of the day
    startDate = new Date(endDate);
    startDate.setDate(endDate.getDate() - 10); // Set startDate 30 days before endDate
  }
  condition.push({
    $match: {
      $expr: {
...
let findOrders = (req, res) => {
  ...
  if (req.query.sellerId) {
    ...
    condition.push({
      $match: { sellerId: new mongoose.Types.ObjectId(req.query.sellerId) },
    });
  }
  ...
  condition.push({ $project: project }, { $sort: sort });
