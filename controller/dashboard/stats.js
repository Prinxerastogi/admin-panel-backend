      ...
      res.status(200).json({
        report: {
          ...
          inDelivery: onlinedeliveryBoysCount.find(
            ...
            ? onlinedeliveryBoysCount.find((x) => x._id === "inDelivery")
            : 0,
          online: onlinedeliveryBoysCount.find((x) => x._id === "online")
            ? onlinedeliveryBoysCount.find((x) => x._id === "online")
            : 0,
          returning: onlinedeliveryBoysCount.find((x) => x._id === "returning")
            ? onlinedeliveryBoysCount.find((x) => x._id === "returning")
            : 0,
          inRoute: onlinedeliveryBoysCount.find((x) => x._id === "inRoute")
            ? onlinedeliveryBoysCount.find((x) => x._id === "inRoute")
            : 0,
        },
      });
    } catch (error) {
