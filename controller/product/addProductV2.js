...
let findProductSku = (req, res, next) => {
    let condition = [
        {
            $match: {
                _id: mongoose.Types.ObjectId(req.body.leafCatId),
            },
        },
        {
            $lookup: {
            ...
            $or: [
                {
                    sku:
                        req.data.code +
                        productSKUlastDigits(req.data.productSku),
                },
                {
...
let createProduct = (req, res, next) => {
    ...
    let insert = {
        ...
        addedBy: {
            id: mongoose.Types.ObjectId(req.decoded.id),
            type: "admin",
        },
    ...
    let condition = [
        {
            $match: {
                $expr: {
                    $in: [
                        req.data.parentProduct._id,
                        "$childProducts.productId",
                    ],
                },
            },
        },
    ...
    productSchema.aggregate(condition, (err, response) => {
        if (err) {
            res.status(400).json({
                success: false,
                message:
                    "something went wrong in checkIsParentExistInChildProducts",
                errMsg: err.message,
            });
        } else if (response.length > 0) {
                ...
                childProducts: [
                    {
                        ...
                        sellPrice: req.body.price,
                        recommendedAttribute: req.data.parentProduct
