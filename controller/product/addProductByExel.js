...
//             'brand.id': (_.brandName) ? mongoose.Types.ObjectId(_.brandName.split('-')[0]) : null,
//             'subBrand.name': _.subBrand ? _.subBrand.split('-')[1] : null,
//             'subBrand.id': (_.subBrand) ? mongoose.Types.ObjectId(_.subBrand.split('-')[0]) : null,
//             shipping: _.shipping ? _.shipping : null,
//             seo: _.seo ? _.seo : null,
//             'addedBy.id': mongoose.Types.ObjectId(req.decoded.id),
//             'addedBy.type': 'admin',
//             categoryId: mongoose.Types.ObjectId(_.leafCatName.split('-')[0]),
//             categories: mongoose.Types.ObjectId(_.leafCatName.split('-')[0]),
//             images: [],
...
let fin1dUrlKey = (req, res, next) => {
    ...
    if (!products || products == undefined || products.length == 0 || products.length == undefined) {
        return res.status(201).json({ success: false, message: 'invalid data ', data: req.body })
    }
    next();
};
...
//     let products = req.body;
//     products.map((_, i) => {
//         _.categoryId = mongoose.Types.ObjectId(_.productDetails.leafCatName.split('-')[0])
//     })
...
let findProductSku = (req, res, next) => {
    let condition = [
        {
            '$match': {
                '_id': mongoose.Types.ObjectId(req.body[0].productDetails.leafCatName.split('-')[0])
            }
        }, {
            '$lookup': {
                'from': 'products',
...
let productsData = (req, res) => {
    ...
    crud.createWthOption(products, {}, productSchema, (err, created) => {
        if (err) return res.status(400).json({ success: false, message: 'error occured in product craetion', err });
        else if (created) {
            return res.status(200).json({ success: true, message: `${products.length} products added`, err });
        }
        else {
...
//         {
//             '$match': {
//                 '_id': mongoose.Types.ObjectId(leafcategoryId)
//             }
//         }, {
//             '$lookup': {
//                 'from': 'products',
