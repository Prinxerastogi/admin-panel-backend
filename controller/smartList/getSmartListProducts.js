...
const getSmartListProducts = async (req, res) => {
    try {
        ...
        if (!smartList) {
            return res.status(404).json({ success: false, message: "SmartList not found" });
        }

        const filter = {};
        if (smartList.config.brand) {
            filter["brand.id"] = mongoose.Types.ObjectId(smartList.config.brand);
        }

        if (smartList.config.subBrand) {
            filter["subBrand.id"] = mongoose.Types.ObjectId(smartList.config.subBrand);
        }

        if (smartList.config.leafCategory) {
            filter["categoryId"] = mongoose.Types.ObjectId(smartList.config.leafCategory)
        } 
        else if (smartList.config.subCategory) {
            const category = await categorySchema.findById(mongoose.Types.ObjectId(smartList.config.subCategory));

            if (category && category.childIds && category.childIds.length > 0) {
                filter["categoryId"] = { $in: category.childIds.map(id => mongoose.Types.ObjectId(id)) };
            }
        } 
        else if (smartList.config.category) {
            const Categories = await categorySchema.find({ parentId: mongoose.Types.ObjectId(smartList.config.category)})
            ...
            if (Categories && Categories.length > 0) {
                ...
                Categories.forEach(childCat => {
                    if (childCat.childIds && childCat.childIds.length > 0) {
                        categoryIds.push(...childCat.childIds.map(id => mongoose.Types.ObjectId(id)));
                    }

                    categoryIds.push(mongoose.Types.ObjectId(childCat._id));
                });
        ...
        const products = await productSchema.aggregate([
                        ...
                        $cond: {
                            ...
                            then: 0,
                                ...
                                $multiply: [
                                    {
                                        $divide: [
                                            { $subtract: ["$sellerProducts.minSellPrice", "$sellerProducts.sellPrice"] },
            ...
            {
                $match: {
                    ...(smartList.config.minPrice || smartList.config.maxPrice || smartList.config.minDiscount || smartList.config.maxDiscount
                        ? {
                            $and: [
                                ...(smartList.config.minPrice ? [{ "sellerProducts.sellPrice": { $gte: smartList.config.minPrice } }] : []),
                                ...(smartList.config.maxPrice ? [{ "sellerProducts.sellPrice": { $lte: smartList.config.maxPrice } }] : []),
                                ...(smartList.config.minDiscount ? [{ "sellerProducts.discountPercentage": { $gte: smartList.config.minDiscount } }] : []),
                                ...(smartList.config.maxDiscount ? [{ "sellerProducts.discountPercentage": { $lte: smartList.config.maxDiscount } }] : [])
                            ]
                        }
                        : {}
                    )
