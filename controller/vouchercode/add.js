...
let generateVoucherCode = (req, res) => {
    if (
        req.data.request.amount == undefined ||
        req.data.request.amount == null
    ) {
        return res
            .status(200)
...
//         if (err) return res.status(400).json({ success: false, message: 'error occured in hash generateVouchercode', err });
//         else {
//             req.data.sCode = hash;
//             //next();
//             create(req, res);
...
//         }
//         else {
//             return res.status(201).json({ success: false, message: 'something went wrong in voucher code', });
//         }
//     })
// };
        ...
        (err, updated) => {
            ...
            if (updated.n > 0 && updated.nModified > 0) {
                ...
                return res.status(400).json({
                    success: false,
                    message:
                        "something went wrong in updatevoucherCodeCashRequest",
                    err,
                });
            }
        }
...
let sendmessageOnMobile = (req, res) => {
    let payload = {
        ...
        body: `Your VoucherCode is:  ${req.data.code}`,
    };
