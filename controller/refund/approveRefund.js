...
const processRefund = async (req, res, next) => {
  try {
    ...
    if (refund.amountSplit.cash > 0) {
      successlog.info(
        "processing cod refund" + refund.amountSplit.cod + " " + refund._id
      );
      cashRefund = await processCodRefund(refund.amountSplit.cash, refund._id);
    }
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      const finalRefundUpdate = await refundSchema.findOneAndUpdate(
        ...
        {
          session,
          arrayFilters: [{ "elem.mode": "wallet", "elem.status": "pending" }],
        }
      );
      if (!finalRefundUpdate) throw new Error("Final refund update failed");
      ...
      if (!orderUpdate) throw new Error("Order wallet money already refunded");
...
const processExpressRefund = async (
  ...
  try {
    ...
    if (data.success) {
      ...
      errorlog.error("easebuzz transfer failed", JSON.stringify(data, null, 2));
      return {
        success: false,
        message: data.data?.transfer_request?.failure_reason || "FAILED",
        data: data,
      };
    }
  } catch (error) {
